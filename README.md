...
- **Custom agents** — define agent personas in **Settings → Agents**, each with a title, instructions, a chosen subset of your installed skills, a model, and a permission mode. Pick one per session from the composer (its instructions shape the prompt, only its skills are visible to it, and its model + permission mode take over), or let the main agent delegate a scoped read-only investigation to one **by name** via `spawn_agent`. **Build one with AI** — describe what you want and the model drafts the persona (name, instructions, model, mode) and suggests stack-relevant skills to install.
- **Agent teams** — group your agents into a **team** for a project (a prebuilt **App Dev Team** — CEO, PM, Architect, Lead Dev, UI/UX, QA, AppSec — ships ready to use). Start a **team project** from Home: the CEO orchestrator runs a shared **task board** (`team_task`) and a living **project brief** (`project_brief`, rendered in a Board/Brief dock panel), delegates research and design to the other roles (who show up as labeled voices in the chat), and **QA + security reviews must pass before a task can close** — a gate enforced in code, not just prompted. Two modes: advisor roles return specs and verdicts while the orchestrator writes; or, with **write-capable builders** (`delegate_build`), designated roles implement autonomously in **isolated git worktrees** whose diffs are merged back for review (parallel-safe, rewindable, and confirmed before each merge and commit).
...
- **Tabbed Settings** — General / Agent / Agents / Teams / Memory / Skills / MCP / Security / About.
