...
src/
  ...
  main/
    ...
    agent/
      ...
      tools.ts          bash, read/write/edit_file, glob, grep, fetch_page, update_plan, memory, skill, spawn_agent
      profiles.ts       Per-model prompts, budgets, apiModel mapping
  ...
  renderer/src/         React 18 UI (App, Chat, Home, Sidebar, RightDock, Settings, TerminalPanel)
...
**No test runner or ESLint/Prettier** in package.json. CI = `npm ci` → typecheck → build (Node 22). Release matrix needs Python setuptools (node-gyp distutils) and Windows MSVC (`ilammy/msvc-dev-cmd`) so `node-pty` postinstall rebuild succeeds.
...
- **Agent tools:** `edit_file` for surgical edits; `write_file` for new files/full rewrites. Read-only tools parallelize; mutations/commands are permission-gated.
...
4. **No automated tests** — verify with `npm run typecheck` and `npm run build` (or `dev` for UI).
