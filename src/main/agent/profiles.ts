...
function teamOrchestrationBlock(team: NonNullable<SystemPromptOpts['teamRole']>): string {
  ...
  return (
    ...
    `3. For each task, delegate the thinking to the right role (Product/Architecture/UI research and design, etc.) via spawn_agent, then implement the result yourself with your edit/command tools. For larger, independent implementation work you can instead delegate_build to a builder role: it writes the code autonomously in an isolated git worktree and its diff is merged back for you to review — commit a baseline first, and use it to parallelize genuinely independent pieces.\n` +
